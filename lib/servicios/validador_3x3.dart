...
class Validador3x3 {
  ...
  static ResultadoValidacion3x3 validarIngresoUI(List<List<Color?>> carasUI) {
    ...
    //    El gestor guarda: [0=Blanco(U), 1=Verde(F), 2=Rojo(R), 3=Amarillo(D), 4=Azul(B), 5=Naranja(L)]
    //    WCA espera:       [U,          R,           F,          D,             L,          B           ]
    //    Entonces:         [caras[0],   caras[2],    caras[1],   caras[3],      caras[5],   caras[4]    ]
    ...
    // Los centros son FIJOS en el cubo 3x3 (no se pueden mover con los movimientos normales).
    ...
    //    Índices en el arreglo plano de 54: [sticker_cara1, sticker_cara2]
    //    Donde cara_i tiene índices [i*9 .. i*9+8], posición dentro de cara:
    //    [0,1,2]
    //    [3,4,5]   (4 = centro)
    //    [6,7,8]
    final List<List<int>> aristas = [
      [1, 46],  // U[1] – B[1]  → Arista UB
      [3, 37],  // U[3] – L[1]  → Arista UL
      [5, 10],  // U[5] – R[1]  → Arista UR
      [7, 19],  // U[7] – F[1]  → Arista UF
      [21, 41], // F[3] – L[5]  → Arista FL
      [23, 12], // F[5] – R[3]  → Arista FR
      [50, 39], // B[5] – L[3]  → Arista BL
      [48, 14], // B[3] – R[5]  → Arista BR
      [28, 25], // D[1] – F[7]  → Arista DF
      [30, 43], // D[3] – L[7]  → Arista DL
      [32, 16], // D[5] – R[7]  → Arista DR
      [34, 52], // D[7] – B[7]  → Arista DB
    ];

    // 6. Mapeo de esquinas (8 piezas de 3 stickers cada una).
    final List<List<int>> esquinas = [
      [0, 47, 36],  // U[0] – B[2] – L[0]  → Esquina UBL
      [2, 45, 11],  // U[2] – B[0] – R[2]  → Esquina UBR
      [6, 18, 38],  // U[6] – F[0] – L[2]  → Esquina UFL
      [8, 20, 9],   // U[8] – F[2] – R[0]  → Esquina UFR
      [27, 24, 44], // D[0] – F[6] – L[8]  → Esquina DFL
      [29, 26, 15], // D[2] – F[8] – R[6]  → Esquina DFR
      [33, 53, 42], // D[6] – B[8] – L[6]  → Esquina DBL
      [35, 51, 17], // D[8] – B[6] – R[8]  → Esquina DBR
    ];
    ...
    for (var idx in esquinas) {
      ...
      debugPrint("  Esquina [$idx]: $cn1-$cn2-$cn3");
