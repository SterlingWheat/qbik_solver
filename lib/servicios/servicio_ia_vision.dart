...
/// Combina YOLOv8 (Detección de objetos) y una CNN Clásica (Clasificación de colores).
class ServicioIAVision {
  static Interpreter? _yoloInterpreter;
  static Interpreter? _cnnInterpreter;
  static bool _modelosCargados = false;

  // --- CONFIGURACIÓN DE MODELOS ---
  static const int _yoloSize = 416; // Tamaño ajustado a tu modelo exportado
  static const int _cnnSize = 64;   // Tamaño de entrada de tu CNN (64x64)
  ...
  /// Carga ambos modelos en memoria la primera vez que se llama.
  static Future<void> _inicializarModelos() async {
    ...
    try {
      ...
      debugPrint("✅ Modelos TFLite cargados correctamente.");
    } catch (e) {
      debugPrint("❌ Error al cargar los modelos TFLite: $e");
      throw Exception("No se pudieron cargar los modelos de IA.");
    }
  }

  /// PROCESO PRINCIPAL: Recibe el fotograma de la cámara y devuelve los 4 colores del cubo 2x2.
  static Future<List<Color>?> procesarFrame2x2(CameraImage imagenCamara) async {
    try {
  ...
  static List<Rect> _ejecutarYOLO(img.Image imagenCentro) {
    ...
    // Leer la estructura exacta de tu modelo para evitar errores de Shape Mismatch
  ...
  static Color _recortarYClasificarCNN(img.Image imagenOriginal, Rect boxNormalizado) {
      ...
      1, (i) => List.generate(
        _cnnSize, (y) => List.generate(
          _cnnSize, (x) {
            ...
            // FÓRMULA DEFINITIVA: Normalización de Transfer Learning (-1.0 a 1.0)
            return [
              (pixel.r - 127.5) / 127.5, 
              (pixel.g - 127.5) / 127.5, 
              (pixel.b - 127.5) / 127.5
  ...
  static List<Rect> _ordenarCajas2x2(List<Rect> boxes) {
  ...
  static List<_YoloBox> _aplicarNMS(List<_YoloBox> cajas, double umbralIoU) {
  ...
  static double _calcularIoU(Rect a, Rect b) {
  ...
  static img.Image? _convertirYUV420aRGB(CameraImage image) {
    try {
  ...
  // Conversor robusto YUV -> RGB en Dart puro con parche de seguridad Android
  static img.Image _procesarYUV420(CameraImage image) {
    ...
    for (int y = 0; y < height; y++) {
      ...
      for (int x = 0; x < width; x++) {
        ...
        // 🛡️ PARCHE DE SEGURIDAD ANDROID: Evita el RangeError
        ...
        // Giro de 90 grados para leer en retrato
