...
class GraficoTiempos extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Detectamos el tema para heredar el contraste correcto
    ...
    return Container(
      ...
      child: CustomPaint(
        painter: _PintorGraficoConEjes(
          tiempos: tiempos,
          colorLinea: Theme.of(context).colorScheme.primary,
          colorTexto: colorTexto,
          esOscuro: esOscuro, // Enviamos el valor booleano correctamente
        ),
      ),
    );
  }
...
class _PintorGraficoConEjes extends CustomPainter {
  ...
  final bool esOscuro; // Propiedad añadida para el control de color dinámico

  _PintorGraficoConEjes({
    required this.tiempos,
    required this.colorLinea,
    required this.colorTexto,
    required this.esOscuro, // Constructor actualizado
  });
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double paddingIzquierda = 45.0; // Espacio reservado para las etiquetas del Eje Y
    final double paddingAbajo = 25.0;     // Espacio reservado para las etiquetas del Eje X
    ...
    for (int i = 0; i <= divisionesY; i++) {
      ...
      // Líneas de la cuadrícula trasera
      canvas.drawLine(
      ...
      // Conversión de milisegundos a segundos visibles (ej: 14.2s)
      ...
      // Pintamos el indicador numérico alineado a la izquierda del grid
    ...
    // Algoritmo dinámico para saltar números en el Eje X si hay demasiados solves (evita solapamientos)
    ...
    for (int i = 0; i < tiempos.length; i++) {
      ...
      if (i == 0 || i == tiempos.length - 1 || i % saltosX == 0) {
        ...
        // Renderizado del número de la resolución abajo
        tpX.paint(canvas, Offset(x - (tpX.width / 2), size.height - paddingAbajo + 8));
        
        // Pequeñas muescas de control estético en la base del gráfico
        canvas.drawLine(
    ...
    final paintPuntoBorde = Paint()
      ..color = (esOscuro ? const Color(0xFF1E293B) : Colors.white) // Corregido: Usa la propiedad inyectada
      ..style = PaintingStyle.fill;
